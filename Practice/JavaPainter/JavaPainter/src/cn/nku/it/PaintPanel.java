...
public class PaintPanel extends JPanel {
	...
	public PaintPanel() {
		addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				p2 = e.getPoint();
				if(Vars.paintControllor.getCurrentShape()== PaintModel.PEN){
					xList.add(p2.x);
					yList.add(p2.y);
					Vars.paintControllor.reportPenList(xList,yList);
					xList.clear();
					yList.clear();
				}else{
	...
	private void drawCurrentShape(Graphics g) {
		...
		}else if(currentShape==PaintModel.PEN){
			shape = new Shape(PaintModel.PEN, shapeColor, xList.toArray(new Integer[0]), yList.toArray(new Integer[0]));
		}else{
	...
	private void drawPen(Shape shape, Graphics g) {
		int[] x = new int[shape.getxArray().length];
		int[] y = new int[shape.getyArray().length];
		
		for (int i = 0; i < x.length; i++) {
			x[i] = shape.getxArray()[i];
			y[i] = shape.getyArray()[i];
		}
